...
public class HeartQueen : Enemy
{
    ...
    [Header("보스 공격 패턴")]
    public Guillotione guillotiones;
    public Gavel gavels;
    ...
    //패턴 별 딜레이
    private IEnumerator PatternDelay(string pattern, float delay) //딜레이 시간, 패턴 이름
    {
        ...
        {
            case "OnGuillotine":
                guillotiones.Init(_player, 1.0f);
                yield return patternDelay;
                break;

            case "OnGavel":
                gavels.Init(_player, 1.0f);
                break;
        }
        ReturnToWalk();
    }
