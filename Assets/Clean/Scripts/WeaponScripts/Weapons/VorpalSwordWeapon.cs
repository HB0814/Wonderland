...
public class VorpalSwordWeapon : WeaponBase
{
    ...
    public VorpalSwordWeaponStats weaponStats;
    ...
    {
        VorpalSwordWeaponStats.LevelStats stats = weaponStats.GetCurrentLevelStats();
    ...
    private void CreateSwordEffect()
    {
        ...
        if (effectObj != null)
        {
            VorpalSwordEffect effect = effectObj.GetComponent<VorpalSwordEffect>();
            if (effect != null)
            {
                effect.Initialize(baseDamage, effectDuration, effectLength);
